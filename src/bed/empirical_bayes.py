...
def log_marginal_likelihood(model, z_lin, x, y, prior_mean, prior_cov, noise_cov):
    ...
    z_lin = jnp.asarray(z_lin).reshape(-1)
    mu0 = jnp.asarray(prior_mean).reshape(-1)
...
def map_estimate(model, x, y, prior_mean, prior_cov, noise_cov, z_init=None, restarts=4, seed=0):
    ...
    d = mu0.size
    L0 = np.linalg.cholesky(np.asarray(prior_cov, dtype=float))
    L0inv = np.linalg.inv(L0)
    yv = np.asarray(y, dtype=float).reshape(-1)
    n = int(jnp.asarray(x).shape[0])
    d_y = yv.size // n
    LR = np.linalg.cholesky(np.asarray(_block_noise(noise_cov, n, d_y)))
    ...
    starts = [mu0] + ([np.asarray(z_init, dtype=float).reshape(-1)]
                      if z_init is not None else [])
...
class EmpiricalBayesResult:
    ...
    # (d_y, d_y) fitted (or kept) observation covariance s * R_0
    noise_cov: jnp.ndarray
    prior_scale: float
    noise_scale: float
    log_marginal_likelihood: float
    # (prior_scale, noise_scale, lml) per iteration
...
def empirical_bayes_init(model, x, y, prior_mean, prior_cov, noise_cov, fit_noise=False, iterations=3,
    ...
    """
    S = jnp.asarray(prior_cov, dtype=float)
    R0 = jnp.atleast_2d(jnp.asarray(noise_cov, dtype=float))
    ...
    for _ in range(int(iterations)):
        z_map, _ = map_estimate(
            model, x, y, mu0, tau * S, s * R0, z_init=z_map)
        z_lin = jnp.asarray(z_map)

        def objective(theta):
            t = jnp.exp(theta[0])
            sc = jnp.exp(theta[1]) if fit_noise else s
            return -log_marginal_likelihood(model, z_lin, x, y, mu0, t * S, sc * R0)

        obj = jax.jit(jax.value_and_grad(objective))
        theta0 = np.array([np.log(tau), np.log(s)]
                          if fit_noise else [np.log(tau), 0.0])
        bounds = [LOG_SCALE_BOUNDS] * (2 if fit_noise else 1)

        def fun(th):
            v, g = obj(jnp.asarray(np.concatenate(
                [th, [0.0]]) if not fit_noise else th))
            g = np.asarray(g)[: len(th)]
            return float(v), g
        res = minimize(fun, theta0[: len(bounds)],
    ...
    lml = float(log_marginal_likelihood(
        model, z_map, x, y, mu0, tau * S, s * R0))
